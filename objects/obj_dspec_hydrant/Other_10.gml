...
if (_s.planted && !_s.launched)
	...
	if (_s.water_time >= 100)
		...
		if (instance_exists(owner) && owner.y < y - 16)
			...
			_ids.water_hitbox_top = hitbox_create_windbox(0, -16, 0.3, 0.3, 0, 2, 90, 61, HITBOX_SHAPE.square, 1, FLIPPER.standard, true, true, false, 8, true);
			_ids.water_hitbox_top.hit_sfx = -1;
			hitbox_set_overlay_sprite(_ids.water_hitbox_top, spr_dspec_hydrant_water, 0, 0.5, 2, 90, c_white, 1, 1);
			}
		else
			{
			var _face = facing;
			...
			_ids.water_hitbox_left = hitbox_create_windbox(-16, 0, 0.3, 0.3, 0, 6, 180, 61, HITBOX_SHAPE.square, 1, FLIPPER.fixed, true, false, true, 0, true);
			_ids.water_hitbox_left.hit_sfx = -1;
			hitbox_set_overlay_sprite(_ids.water_hitbox_left, spr_dspec_hydrant_water, 0, 0.5, 2, 180, c_white, 1, 1);
			_ids.water_hitbox_right = hitbox_create_windbox(16, 0, 0.3, 0.3, 0, 6, 0, 61, HITBOX_SHAPE.square, 1, FLIPPER.fixed, true, false, true, 0, true);
			_ids.water_hitbox_right.hit_sfx = -1;
			hitbox_set_overlay_sprite(_ids.water_hitbox_right, spr_dspec_hydrant_water, 0, 0.5, 2, 0, c_white, 1, 1);
			facing = _face;
			}
		}
...
if (!_s.planted)
	...
	repeat(abs(vsp))
		...
		if (!on_ground(x, y, vsp)) then y += sign(vsp);		
			...
			//Create hurtbox
			_ids.hurtbox = hurtbox_create(spr_dspec_hydrant_obj);
			with(_ids.hurtbox)
				{
			...
			//Visual effects
			fx_create(spr_dust_impact, 1, 0, 21, x, bbox_bottom + 1, 1, 90, "FX_Layer_Below");
			break;
			}
		}
	}
	...
	var _results = entity_move(hsp, vsp, false, true, 0.9, false);
	hsp = _results.hsp;
	vsp = _results.vsp;
	}
